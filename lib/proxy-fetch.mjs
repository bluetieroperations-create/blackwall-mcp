...
// Hard cap on a buffered response body. A risk verdict is a few KB of JSON; a
// proxy in a default-deny sandbox is the (untrusted) egress point and could
// otherwise stream an unbounded body, exhausting memory before the timeout
// fires. Past this many bytes we destroy the socket and fail closed. Override
// via BLACKWALL_MAX_RESPONSE_BYTES (floor 64 KiB) for unusually large verdicts.
