...
import { getProxyForUrl, proxyFetch } from './proxy-fetch.mjs';
...
export async function forecast({ action, inputs, context, depth, parent_forecast_id }, opts = {}) {
  ...
  // Route through HTTPS_PROXY when set (Node's fetch ignores it) so the gate can
  // reach the API from default-deny / proxy-only-egress networks (e.g. NemoClaw).
  const proxy = opts.fetch ? null : getProxyForUrl(url);
  const fetchImpl = opts.fetch ?? (proxy ? proxyFetch(proxy) : globalThis.fetch);
